...
def getLoginDetails():
    ...
    conn.close()
    first_name=list(chain.from_iterable(firstName))
...
def deleteWish():
    ...
    with sqlite3.connect(db_path) as conn:
        cur = conn.cursor()
        cur.execute("SELECT product_id FROM wishlist")
...
def cart():
    ...
    with sqlite3.connect(db_path) as conn:
        ...
        userId = cur.fetchone()[0]
        cur=conn.cursor()
        cur.execute("SELECT DISTINCT user.product_name FROM user, cart WHERE user.product_id = cart.product_id AND cart.userId = ?", (userId, ))
        name=cur.fetchall()
        cur.execute("SELECT DISTINCT user.product_price FROM user, cart WHERE user.product_id = cart.product_id AND cart.userId = ?", (userId, ))
        price=cur.fetchall()
        cur.execute("SELECT DISTINCT user.product_url FROM user, cart WHERE user.product_id = cart.product_id AND cart.userId = ?", (userId, ))
        image=cur.fetchall()
    ...
    a=a[0]
    price_final=list(chain.from_iterable(price))
    price_int=[x[1:]for x in price_final]
    price_int=[float(x) for x in price_int]
    image_final=list(chain.from_iterable(image))
    ...
    if total==0:
        ...
        total="£"+str(total)+"0"
    return render_template("cart.html",name= name_final,price=price_final,image=image_final,a=a,size=capital,quantity=number_final,product_id=product_id,total=total,loggedIn=loggedIn,firstName=firstName,total_quantity=total_quantity)
